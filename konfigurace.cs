...
namespace Labels
{
    ...
    class Configuration
    {
        ...
        private const string AppName = "Tisk štítků na EPL tiskárně v.202507";
        ...
        private static void Initialize()
        {
            ...
            int align = 72;
            string infoLine;
            if (string.IsNullOrEmpty(MasterTemplateAddress))
            {
                infoLine = $"Adresář se soubory: {Path.GetFullPath(TemplatesDirectory)}";
            }
            else
            {
                infoLine = $"Vstupní data: {Path.GetFullPath(MasterTemplateInputAddress)}";
            }

            string pathToConfigFile = Path.Combine(ConfigPath, ConfigFile);
            if (pathToConfigFile.Length > align - "Konfigurační soubor: ".Length - 2)
            {
				// Zkrátí cestu ke konfiguračnímu souboru, pokud je příliš dlouhá
				int keepLength = align - "Konfigurační soubor: ".Length - 5; // 5 pro "..\" a případné oddělovače
				string fileName = Path.GetFileName(pathToConfigFile);
				if (fileName.Length + 5 < keepLength)
				{
					// ponechá začátek, "..\", a konec (název souboru)
					int startLength = keepLength - fileName.Length;
					pathToConfigFile = pathToConfigFile.Substring(0, startLength) + "..\\" + fileName;
				}
				else
				{
					// pokud je i název souboru moc dlouhý, zkrátí i ten
					pathToConfigFile = "..\\" + fileName;
				}
            }
            Header = string.Format(
                "{6}{0}" +
                "Konfigurační soubor: {1}" +
                "Adresa tiskárny: {2}{0}" +
                "{3}" +
                "Typ tiskárny: {4}{0}" +
                "Kódování souborů: {5}" +
                "{7}{0}",
                Environment.NewLine,
                pathToConfigFile.PadRight(align - "Konfigurační soubor: ".Length),
                PrinterAddress,
                infoLine.PadRight(align),
                PrinterTypeByWords,
                Encoding.PadRight(align - "Kódování souborů: ".Length),
                AppName,
